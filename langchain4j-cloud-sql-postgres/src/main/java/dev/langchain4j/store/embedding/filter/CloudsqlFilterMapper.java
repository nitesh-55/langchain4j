...
public class CloudsqlFilterMapper {
  ...
  private String mapGreaterThan(IsGreaterThan isGreaterThan) {
    return String.format(
        "\"%s\" > %s", isGreaterThan.key(), formatValue(isGreaterThan.comparisonValue()));
  }

  private String mapGreaterThanOrEqual(IsGreaterThanOrEqualTo isGreaterThanOrEqualTo) {
    return String.format(
        "\"%s\" >= %s",
        isGreaterThanOrEqualTo.key(), formatValue(isGreaterThanOrEqualTo.comparisonValue()));
  }

  private String mapLessThan(IsLessThan isLessThan) {
    return String.format(
        "\"%s\" < %s", isLessThan.key(), formatValue(isLessThan.comparisonValue()));
  }

  private String mapLessThanOrEqual(IsLessThanOrEqualTo isLessThanOrEqualTo) {
    return String.format(
        "\"%s\" <= %s",
        isLessThanOrEqualTo.key(), formatValue(isLessThanOrEqualTo.comparisonValue()));
  }
  ...
  private String mapNotIn(IsNotIn isNotIn) {
    ...
    return String.format(
        ...
        key, key, formatValuesAsString(isNotIn.comparisonValues()));
  }

  private String mapContainsString(ContainsString containsString) {
    return String.format(
        "\"%s\" ILIKE '%%%s%%'",
        containsString.key(), formatValue(containsString.comparisonValue()));
  }
  ...
  String formatValuesAsString(Collection<?> values) {
    return "("
        + values.stream().map(v -> String.format("'%s'", v)).collect(Collectors.joining(","))
        + ")";
  }
}
